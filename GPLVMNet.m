classdef GPLVMNet < handle
    properties
       W1, B1, W2, B2, W3, B3, W4, B4, W5, B5, W6, B6;
       Affine1, Affine2, Affine3, Affine4, Affine5, Affine6;
       Sigmoid1, Sigmoid2, AddLayer;
       ReLu1, ReLu2, ReLu3, ReLu4,ReLu5, ReLu6,ReLu7;
       SquareError, LogLikelihood_GPLVM;
       SigmoidDualOutput;
       gamma, MullLayler;
    end
    methods
        function [self, param] = GPLVMNet(X, m, gamma)
            ...
            self.LogLikelihood_GPLVM = LogLikelihood_GPLVM();
        ...
        function L = loss(self, X, T, param, x_gplvm)
            [Y, lowdim] = self.predict(X);
            L1 = self.SquareError.forward(Y, T);
            L2 = self.LogLikelihood_GPLVM.forward(lowdim, param, x_gplvm);
        ...
        % Analytical Gradient ------------------------------------------
        function grads = analytical_gradient(self, X, T, param, x_gplvm)
            % forward propagation
            self.loss(X, T, param, x_gplvm);
            ...
            doutGP = self.LogLikelihood_GPLVM.backward(doutGP);
            ...
            grad_GPLVMparam = self.LogLikelihood_GPLVM.dparam_param;
            grad_GPLVMinput = self.LogLikelihood_GPLVM.dparam_input;
            
            grads = {grad_W1 grad_B1 grad_W2 grad_B2 grad_GPLVMparam grad_GPLVMinput};
